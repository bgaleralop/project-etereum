...
class OpenImageUseCase @Inject constructor(
    ...
    suspend operator fun invoke(uri: Uri): Result<ImageProcessResult> = withContext(Dispatchers.IO) {
        ...
        return@withContext if (image.isSuccess) {
            ...
            Result.success(ImageProcessResult(bitmap, bitmap.allocationByteCount / 1024, isSanitized = false))
        } else {
